...
- 画面上半分の右端（カエデの横の空きスペース）：導線ボタン4つを縦並びで配置。上から「プレゼント」「デイリーミッション」「図鑑」「設定」（[共通UIボタン素材](../ui-buttons.md)の`ui_present` / `ui_daily_missions` / `ui_guide` / `ui_settings`を使用、56px角）。**配置のみで、タップしても何も起きない**（各機能は未実装）
