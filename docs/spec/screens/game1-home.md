...
パーティ編成の仕組みがまだ無いため、**現状は`home_04`（4人とも揃った版）で固定**している（テスト目的、`HOME_BACKGROUND`定数）。編成の仕組みができたら、人数に応じて動的に切り替える。
...
- 「⚔ 戦闘テスト（仮）」ボタン：[戦闘画面・動作確認版](./battle-test.md)（`/games/game1/battle`）への仮の入り口。正式な「冒険」の導線ができたら撤去する
